...
Goal: decouple sim from render. Move the simulation into a dedicated Web Worker so the main thread does nothing but render. After v1.6, frame rate is purely a function of render cost, and sim throughput is purely a function of tick cost — they no longer share a RAF callback. Achieved via two SharedArrayBuffers (control + double-buffered snapshot) and `Atomics.waitAsync` + futex pacing, with JS-side frustum cull replacing the deleted Rust pack-render-buffer path.

Mission: `docs/plans/v1.6-mission.md`. Plan: `docs/plans/v1.6-plan.md`. Plan-review: `docs/plans/v1.6-plan-review.md`. Implementer guide: `docs/plans/v1.6-implementer-guide.md`. No user override sheet — the review's recommendations landed as locked decisions.

Pre-flight: the user's working tree at orchestration start contained two coherent strands of dev work not related to v1.6 sim/render decoupling — they shipped as their own commits before Wave A so the plan's deletion list (which named `set_digestion_cooldown` / `set_repulsion_max`) stayed accurate.

Commits in landing order on main (waves interleave with two strands of user-initiated pre-v1.6 polish that overlapped with the orchestration window):

- 5214ba2 `feat(sim+ui): digestion_cooldown + repulsion_max sliders; grass→white+opacity; grass step-split` — pre-v1.6 polish: promote two hardcoded constants to DevSliders + typed setters; grass shader white + `u_opacity` uniform driven by Settings.grassOpacity; settings panel width driven by `--settings-w` CSS var; profiler split `grass.upload` into `.read` (wasm→JS quantize) vs `.gpu` (texImage); `GrassGrid::step` split into `compute_propagation` + `rebuild_row_bitset` so the World profiler can break the two phases apart.
- be1fd01 `docs(plans): archive v1.3/v1.4 plans, retire v1.5, add v1.6 mission/plan/guide` — move shipped plans to `docs/archive/`; drop v1.5 planning doc set (DECISIONS.md captures it); land v1.6 mission, mission-review, plan, plan-review, implementer-guide, orchestrator-prompt.
- f6990a4 `feat(web): sim-bridge.ts — message protocol types + SAB layout constants` — **Wave A2**. New `web/src/sim-bridge.ts` (1 file, 292 LOC). `SIM_BRIDGE_VERSION=1`, `MAX_POP_FOR_SAB=8000`, `CREATURE_STRIDE=8`, `SNAPSHOT_HEADER_BYTES=32` (20 stats + 12 byte pad to 32-byte align the creature SoA — typed-array offset alignment is browser-enforced; do NOT trim), `SLOT_BYTES=486_432`, `SNAPSHOT_SAB_BYTES=972_864`. Discriminated unions for all 10 main→worker + 5 worker→main message shapes. `readSnapshotHeader` decodes `tps` via `getFloat32(off+12, true)` to round-trip Rust's `f32::to_bits` write. Type-only — no runtime imports yet.
- e1e5d12 `fix(grass): gate dead step() wrapper to #[cfg(test)]` — orchestrator pre-Wave-A fix: 5214ba2's grass split left the old `step()` wrapper publicly visible but only tests call it; clippy `-D warnings` rejected. `#[cfg(test)]` collapses it to test-only without losing the convenience helper.
- cec1180 `feat(wasm): write_snapshot_to + id column in creature SoA (stride 6→8)` — **Wave A1**. `creature_stride()` 6→8. Per-creature SoA now `[x, y, body_radius, color_r, color_g, color_b, id_lo_as_f32_bits, id_hi_as_f32_bits]`. New `write_snapshot_to(creatures_dst, grass_dst, stats_dst)` wasm-bindgen method (wasm-only body; native `write_snapshot_to_native` helper covers the unit test). Stats header is 5 × 4 bytes LE: `[tick, pop, world_ended, tps_bits = f32::to_bits, jank_count]`. Truncation log-warned once per WorldHandle when pop > MAX_POP_FOR_SAB. New `max_pop_for_sab()` free export so Wave B can handshake-assert against the TS const. Renderer stride guard tightened to `6 || 8` (the tolerant A1→B bridge — closed in Wave B). **No `cross_origin_isolated()` Rust export and no Rust-side snapshot profile span** — both per plan-review (worker-side spans would orphan at root level; isolation is read JS-side).
- 4b733fa / 3f4af3a `Merge worktree-a{1,2}` — Wave A reconciled. Gate suite green; renderer boots with tolerant stride guard.
- 18cdbd1 `feat(world): scale world 600u→1200u (4× area, 4× grass cells)` — pre-v1.6 polish: `WORLD_SIZE 600.0 → 1200.0`. Auto-derived `HASH_DIM 120→240`, `GRASS_GRID_DIM 480→960`, `GRASS_CELL_COUNT 230_400→921_600`. `sim-bridge.ts::GRASS_BYTES` kept in sync so v1.6's SAB layout reserves the right per-slot grass region (now 921 KB instead of 230 KB).
- 37b1b2f `feat(profile): per-layer NN forward + per-chunk grass propagation timings` — pre-v1.6 polish: 3 new profile siblings under `tick.nn` (`forward.l1/l2/l3`) + 3 under `tick.grass_step.compute` (`compute.par_chunks`, `compute.chunks_mut`, `compute.row_body`). `Brain::forward` gains an optional `&mut PickTimings` so the SIMD forward pass can break into its 3 matmul phases without paying the clock-read cost when profiling is off.
- b4677b0 + 936f6f9 `feat(arch): move sim into Web Worker (Stage 1 postMessage snapshots)` — **Wave B**. 13 files; 1 new (`web/src/sim-worker.ts`, ~300 LOC); main.ts gutted from wasm-holding to thin render shell. Sim worker imports `init, WorldHandle, max_pop_for_sab, initThreadPool` from `../wasm/evosim`; reads `crossOriginIsolated` from `self`; runs ONE tick + writes ONE postMessage snapshot before posting `boot_ready` (first-paint handshake — no zero-pop ghost frame); routes all writes through `world.set_slider(name, value)` including `auto_curriculum` (the only bool DevSlider) as `0|1`. `try_set_slider` gains the matching `"auto_curriculum"` arm so Wave C's deletion of `set_auto_curriculum` doesn't strand any caller. `currentSliderState()` reads in-memory widget state, not `getSettings()`, so a mid-drag restart carries the dragged value. `__world` debug hook deleted. `stats_sample` Rust path replaced by `SnapshotHeader` reads on main; stats-sampler guard changed from `tick % 10 === 0` to `tick - lastSampledTick >= 10` (mod-10 would skip multi-boundary batches at high TPS). Worker-stats / perf-panel poll via 750 ms / 1 s request-reply. **Wave-B deviation logged in the implementer report**: with `--features threads`, wasm linear memory IS a SharedArrayBuffer, so `postMessage(typedArrayView)` shares the SAB rather than copying — the agent worked around by explicitly copying view contents into a fresh non-shared `ArrayBuffer` before posting. Entire copy path goes away in Wave C. Stage 1 acceptance ceiling pop ≤ 1500; Wave C closes the structured-clone gap.
- d11143a `style: cargo fmt drift from per-layer profile commit` — orchestrator post-Wave-B fmt fix on brain.rs + grass.rs.
- d950627 `fix(worker): route profiler clock through scope-agnostic performance.now()` — **Wave B blocker fix**. Sim worker panicked at boot with `panicked at src/profiler.rs:426:10: "no window"`. `clock_now_ms()` / `clock_now_us()` were calling `web_sys::window()` which returns `None` in a Worker scope. Moved the existing `ts_perf` `wasm_bindgen` `js_namespace = performance` binding above the call sites and routed both through it — works identically in window, worker, and rayon-worker scopes.
- d45148e `fix(worker): route wasm_api.rs wall clock through threadsafe binding too` — second symptom of the same bug; `wasm_api.rs::wasm_now_ms()` had the same `web_sys::window()` call at WorldHandle ctor's `step_n` jank-budget timer. Reuse profiler's `clock_now_us_threadsafe()`.
- 14ee4e3 + 3a0f04f `perf(arch): SharedArrayBuffer snapshots — zero-copy render path` — **Wave C**. Replace Wave B's postMessage snapshot (and the explicit-copy workaround above) with two SABs: control SAB (`Int32Array(4)` — current_slot, seq, futex, reserved) + double-buffered snapshot SAB (~974 KB). Sim worker writes inactive slot via `world.write_snapshot_to(...)` inside a TS-side `worker.snapshot.write` span, then `Atomics.store(ctrl, CTRL_CURRENT_SLOT, new_slot)` → `Atomics.add(ctrl, CTRL_SEQ, 1)` (store-before-add so main's `Atomics.load` of current_slot is coherent on every seq tick). Main render loop builds `Float32Array` / `Uint8Array` views over the live slot inside a TS-side `frame.snapshot.read` span. Renderer takes raw views + pop; JS-side frustum cull + GPU instance pack lives in `render-gl.ts` (replaces what `pack_render_buffer` did Rust-side). Highlight pass reads ids via `Uint32Array` view at offset+24 of each 32-byte creature stride. **Deletions**: `creatures_buffer`, `grass_buffer`, `grass_buffer_u8`, `creature_ids_buffer`, `pack_render_buffer`, `creature_stride`, `stats_sample`, and all 21 per-typed slider setters (`set_mutation_rate_multiplier`, …, `set_auto_curriculum`). `set_slider(name, value)` name-dispatcher is the sole external entry point post-decoupling. `apply_*` private helpers stay (called by `try_set_slider`). `src/wasm_api.rs` shrinks 366 LOC. Wasm bundle 623 KB → 596 KB.
- 89c683a `feat(profile): add grass_step.compute.row_body.self per-closure self-time` — pre-Wave-D polish: `row_body_self_us` measures from inside the closure body, excluding the per-chunk `chunks_mut` dispatch overhead (`row_body_us` includes it). Two siblings under `tick.grass_step.compute.row_body` for clean attribution.
- d48df4f + c6f1f74 `feat(arch): Atomics.waitAsync pacing + batched step_n + inspector fast-path` — **Wave D**. Replaces Stage-1's `setTimeout(0)` pacing (Chrome 4 ms minimum → ~250 TPS ceiling at any target) with `async function simLoop()` body that `await`s `Atomics.waitAsync(ctrl, CTRL_FUTEX, before, timeoutMs).value`. **The `await` is what allows the event loop to run between iterations**, so `onmessage` callbacks fire and sliders / pause / restart / inspect all flow — synchronous `Atomics.wait` would dark-hole them all (the v1.6 plan-review caught a buggy pseudocode draft that wrote `Atomics.wait` and would have shipped this exact bug). `step_n(batch)` accumulator: `budget += targetTPS * elapsed`; `batch = floor(budget)`; `MAX_TICKS_PER_BATCH = 2000`. Slider drain ordering: queue drained at TOP of every iteration, before `step_n` and before SAB write — slider sent at tick T takes effect for tick T+1, deterministically. When paused, `timeoutMs = Infinity`; worker parks until `set_paused(false)` notifies the futex. Main wake side: every `SimBridge.postMessage` does `Atomics.add(controlI32, CTRL_FUTEX, 1)` then `Atomics.notify(controlI32, CTRL_FUTEX, 1)`; the `add` mutates the futex so an about-to-park `Atomics.waitAsync` returns synchronously with "not-equal". Per-name slider debouncer in `sim-bridge.ts` (16 ms trailing edge; last value wins) prevents pointermove flooding. Inspector SAB fast-path: click → JS-side scan of SAB id column for nearest creature within tolerance → render SoA fields immediately (x, y, color_ema) → async-request `inspect_id` for the full JSON; per-frame refresh only re-requests if panel open AND > 250 ms since last reply (~4× traffic reduction). `web/tsconfig.json` adds `"ES2024.SharedMemory"` lib for `Atomics.waitAsync` types. New `rayon_current_num_threads()` wasm-bindgen export; sim worker warns "rayon collapsed to 1 thread" if `<= 1` after `initThreadPool` (v1.5 silent-single-thread footgun defense).

Wave E runtime verification (Playwright headless via `/tmp/wave-e-probe.mjs`):
- (a) `[threads] main thread: SharedArrayBuffer=true crossOriginIsolated=true` ✓
- (b) `[sim] crossOriginIsolated=true` + `[sim] rayon workers: 20` (worker + main mirror) ✓
- (c) `grep -c initThreadPool web/wasm/evosim.js` = 2, `grep -F 'shared:true'` matches ✓
- (d) FPS 60.5 at pop=7323 (well above the ≥55 at pop>1500 ceiling) ✓
- (e) TPS scrub 60 → 1000 → 60: FPS stable at 60.5 throughout (no camera stutter) ✓; observed sustained TPS at 1000 dropdown: 199.6 at pop ~7300 (bound by per-tick wasm cost, not by pacing — Stage 1's ~250 cap was pacing-bound; Wave D removes it).
- (g) Restart blip via `r`: 292 / 373 / 355 ms (target 500 ms, ceiling 800 ms) ✓
- (h) Hammer-restart 5× `r` in 5 s: 0 new console errors ✓
- (f), (i), (j) deferred — auto-probing the inspector text format, the handshake-mismatch assertion, and the profile_report_json capture require more involved DOM/UI driving than the headless probe could clean-room.

Surfaces v1.6 changed (canonical pointers for future readers):
- Sim worker: `web/src/sim-worker.ts` (new). Owns the single wasm instance, the rayon pool, the message queue, and the `Atomics.waitAsync` loop.
- Bridge: `web/src/sim-bridge.ts` — message protocol discriminated unions + SAB layout constants + `SimBridge` class (correlation map for request/reply + per-name slider debouncer + futex wake).
- Shared memory: control SAB (16 bytes — current_slot, seq, futex, reserved) + double-buffered snapshot SAB (~974 KB; per slot = 20-byte stats header + 12-byte pad + 256 KB creatures + 922 KB grass).
- Render: `web/src/render-gl.ts` JS-side frustum cull + GPU instance pack reads SAB views; `pack_render_buffer` deleted.
- WorldHandle surface: `write_snapshot_to`, `max_pop_for_sab`, `rayon_current_num_threads`, `set_slider` name-dispatcher, `creature_at` / `creature_idx_by_id` / `creature_inspect_json` (for inspector replies). Everything else is gone.

Deferred to v1.7+ (per `v1.6-mission.md` §"Non-goals"):
- GPU-side grass propagation.
- Sim hot-swap / save-load.
- Inspector subscription streaming (Wave D shipped request/reply + 250 ms throttle; streaming is a later optimization).
...
Goal: profiler / perf-instrumentation overhaul. Four sibling top-level trees (`frame`, `tick`, `nn`, `grass_step`) instead of one big tree rooted at `tick` — surfaces the structural difference between *main sim worker wall-clock wait* (`tick.nn`, `tick.grass_step` as leaves) and *sum-busy across all rayon workers* (top-level `nn`, `grass_step` with their full breakdown). Every node uses its full dotted prefix; every parent row has a real timer pair (no rollups). `frame` gets an outer RAII span around the whole RAF callback so unaccounted-for render overhead surfaces in the parent instead of hiding in a rollup.

Mission: `docs/plans/v1.7-mission.md`. No separate plan doc / plan review / implementer guide — the mission was small enough to ship from straight (one ~1-day implementer pass instead of v1.6's multi-wave pipeline).

Commits in landing order on main:

- 0a82bd4 + 508968b `refactor(profile): four-tree profiler (frame, tick, nn, grass_step) with real parent spans` — single implementer commit (9 files, +486/-174 LOC). New `Profiler::record_under_root(root_name, path, dur_us)` + `push_root_named(name)` + `ensure_root(name)` generalize the old `record_under_tick` so the four trees coexist as siblings. Pre-seeded roots: `tick` (ROOT_TICK=0, Rust spans), `frame` (ROOT_FRAME=1, TS spans stitched in on the panel side); `nn` and `grass_step` minted lazily via `ensure_root` so they show up only when the worker-sum trees have data. Parent rows in the worker-sum trees (`nn`, `nn.build_input`, `nn.proximity`, `nn.forward`, `grass_step`, `grass_step.row_compute`) get their own per-worker atomic accumulators (e.g., new `NnStats::tick_build_input_total_us`, `tick_proximity_total_us`) bracketing the parent operation in each worker — NOT computed from children's rollups. `tick.nn` and `tick.grass_step` become leaves in the `tick` tree (raw rayon-dispatch wall time only); the deeper breakdown lives only under the top-level worker-sum trees. `tick.color_ema` lifted out of `tick.bookkeeping_tail` to sibling-of-bookkeeping_tail per the mission's tick-tree diagram. `grass_step` children renamed under the user-approved scheme (`par_chunks`/`chunks_mut` → `dispatch`, `row_body` → `row_compute`, `row_body.self` → `row_compute.body`, `rebuild_row_bitset` → `bitset_rebuild`). Multi-root API choice was approach (a) from the mission ("`record_under_root` flavour of atomic accumulators") because the existing `clock_now_us_threadsafe() + fetch_add` pattern crosses the rayon worker boundary cleanly — approach (b) would have required teaching the non-`Send`/`Sync` `Profiler` to live inside worker closures.

TS side: `web/src/perf.ts::span("frame")` special-cases the empty-stack frame case so the outer-frame RAII span attaches to the pre-seeded root rather than minting a duplicate row. `web/src/render-gl.ts` adds `frame.render_world` + `.grass` + `.creatures` spans inside `renderWorld`. `web/src/widgets/perf-panel.ts` rewritten for the 4-tree stacked layout — four `.profiler-tree-section` blocks, each with its own `.profiler-tree-header`, table columns `path · total ms · calls · ms/call · share %`, indentation by dotted-prefix depth, insertion-order stable (no alpha-sort), `(no samples yet)` placeholder for empty trees. `web/index.html` replaces the single `<table>` with a `#profiler-trees` container; new `.profiler-tree-section`/`.profiler-tree-header` styles in `web/src/styles.css`.

Wave E runtime verification (Playwright headless at pop ~1500, TPS=1000, profiler enabled near-startup so the 1 Hz request-reply cadence has time to populate):

- **All 4 trees populate**: ✓ `frame`, `tick`, `nn`, `grass_step` all present with non-empty row sets after ~8 s warmup.
- **No `0.00 ms` parent rows**: ✓ at sample, `nn=78.96`, `nn.build_input=30.93`, `nn.proximity=21.64`, `nn.forward=40.56`, `grass_step=1246.04`, `grass_step.row_compute=6696.85`, `grass_step.row_compute.body=6502.08`, `grass_step.bitset_rebuild=159.04`, `frame=115.20`, `frame.render_world=92.39` — all positive on healthy ticks.
- **`tick.nn` and `tick.grass_step` are leaves**: ✓ probe confirmed no `tick.nn.*` or `tick.grass_step.*` children in the rendered tree.
- **No legacy node names**: ✓ `grep -rn 'tick\.nn\.\|tick\.grass_step\.compute\.\|grass_step\.compute' src/ web/src/` returns no matches.
- **Outer `frame` shows unaccounted overhead**: ✓ `frame.total=115.20` vs children-sum=99.65 = ~15.6 ms RAF overhead surfacing in the parent (the diagnostic value the mission asked for).
- **`nn.forward / tick.nn ≈ N_workers` (success criterion #2)**: observed 0.18 at pop ~1500 with 20 hardware threads — **NOT ≈ 20 as the criterion expected**. Implementation is correct; the criterion was idealistic. Reason: `nn.forward` only times the L1+L2+L3 forward layers (a small slice of per-chunk work), and `tick.nn` includes rayon dispatch + restore + atomic-store overhead. The better proxy is `nn(root) / tick.nn = 78.96 / 223.77 ≈ 0.35` (sum-busy of all NN work vs wall) — still well under 20 because at pop ~1500 there are only ~75 creatures/worker, too small to amortize rayon dispatch overhead. The criterion would hold at much higher pop where per-chunk work dwarfs dispatch.

Surfaces v1.7 changed (canonical pointers for future readers):
- Multi-root profiler: `src/profiler.rs::Profiler::record_under_root(root_name, path, dur_us)` is the hot path for atomic-accumulator-driven trees (`nn`, `grass_step`); `push_root_named(name)` is the RAII flavour for spans that bracket a root operation. `ensure_root(name)` is the lazy mint.
- Per-worker parent brackets: `NnStats::tick_build_input_total_us` / `tick_proximity_total_us` hold the parent timer pairs; the existing per-leaf `*_us` counters stay as leaves.
- Display: `web/src/widgets/perf-panel.ts` renders the 4-tree stacked layout — DOM root is `#profiler-trees` in `web/index.html`; per-tree section class is `.profiler-tree-section`.

Known limitations carried over (not v1.7 work; flagged for future):
- **Sim-worker-side `worker.snapshot.write` span is silently dead**. The worker's TS perf module is a separate instance from main's; main never reads worker-side spans, so the span has been a no-op since Wave C. The agent left it untouched per mission scope; fixing it needs either a worker→main perf-message channel or relocating that span to wrap the postMessage on the receiving side.
