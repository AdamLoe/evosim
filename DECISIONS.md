...
D10 (species tracking deletion): deleted `src/species.rs` entirely; removed `SpeciesRegistry`, `live_species_count`, `peak_species_count`, `pending_extinction_check`, `species_id`/`parent_species_id` SoA fields, `finalize_extinctions()`, 4 species constants, `species_count`/`species_list_json` wasm exports, species panel in rail (SpeciesRow, renderSpeciesList, 1 Hz poll), species chart in stats.ts, species fields from inspector.ts + index.html; stubbed `species_name: String::new()` in HoF literals (D5 will delete); added `#[allow(dead_code)]` on Speciation/Extinction event variants (D4 will delete); removed peak_species from WorldEnded event; fixed pre-existing `initThreadPool` TS2614 by importing via runtime cast. Deleted 7 species-related tests. All gates: cargo fmt/build/test-lib 181/183 (seq/threads), clippy both features, pnpm typecheck + build. No golden regen needed (species fields were not in snapshot hash input paths exercised by the acceptance seed).
