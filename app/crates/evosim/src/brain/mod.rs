...
impl Brain {
    ...
    /// Founder-only action-output boost. Scales the output-layer weight rows
    /// feeding the **Graze** logit (`out[2]`) by `graze_mult` and the
    /// **Split/Mate** logit (`out[4]`) by `split_mult`. (Recall `out[0..2]` are
    /// the velocity outputs and `out[3]` is Attack — those rows are untouched.)
    ///
    /// The brain has no per-output bias term (outputs are pure weighted sums of
    /// hidden activations), so scaling a row inflates that logit's variance,
    /// making it win the action argmax more often — a lean toward grazing /
    /// splitting / mating for brand-new creatures. Applied ONCE at construction;
    /// applying it per birth would compound `kᴳ` across generations. Both
    /// multipliers at 1.0 is a no-op.
    pub fn boost_action_outputs(&mut self, graze_mult: f32, split_mult: f32) {
        if graze_mult == 1.0 && split_mult == 1.0 {
            return;
        }
        // Output matmul: fan_in = last hidden width (or input width if no
        // hidden layers), fan_out = NN_OUTPUTS. Rows are fan_in-contiguous,
        // matching `matmul_output`'s `weights[o*fan_in..]` indexing.
        let fan_in = self
            .topology
            .hidden_sizes()
            .last()
            .copied()
            .unwrap_or_else(|| self.topology.input_width());
        let out_off = self.topology.weight_count() - fan_in * NN_OUTPUTS;
        // Action logit o maps to output row (2 + o): Graze=out[2], Split=out[4].
        for (row, mult) in [(2usize, graze_mult), (4usize, split_mult)] {
            if mult == 1.0 {
                continue;
            }
            let base = out_off + row * fan_in;
            for w in &mut self.weights[base..base + fan_in] {
                *w *= mult;
            }
        }
    }
...
mod tests {
    ...
    #[test]
    fn boost_action_outputs_scales_only_graze_and_split_rows() {
        let mut rng = SimRng::from_u64(7);
        let topo = NnTopology::legacy();
        let fan_in = *topo.hidden_sizes().last().unwrap(); // 24
        let total = topo.weight_count();
        let out_off = total - fan_in * NN_OUTPUTS;
        let baseline = Brain::founder(&mut rng, topo);
        let mut boosted = baseline.clone();
        boosted.boost_action_outputs(2.0, 3.0);

        // Output rows: 0=vx, 1=vy, 2=Graze, 3=Attack, 4=Split/Mate.
        for row in 0..NN_OUTPUTS {
            let base = out_off + row * fan_in;
            let expect_mult = match row {
                2 => 2.0,
                4 => 3.0,
                _ => 1.0,
            };
            for k in 0..fan_in {
                assert_eq!(
                    boosted.weights[base + k],
                    baseline.weights[base + k] * expect_mult,
                    "row {row} slot {k}"
                );
            }
        }
        // Hidden-layer weights (everything before the output matmul) untouched.
        assert_eq!(&boosted.weights[..out_off], &baseline.weights[..out_off]);
    }

    #[test]
    fn boost_action_outputs_unity_is_noop() {
        let mut rng = SimRng::from_u64(9);
        let b = Brain::founder(&mut rng, NnTopology::legacy());
        let mut b2 = b.clone();
        b2.boost_action_outputs(1.0, 1.0);
