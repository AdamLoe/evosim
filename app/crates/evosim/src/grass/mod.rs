...
use crate::rng::{grass_hash_fused_4, grass_hash_u64, SimRng};
...
impl GrassGrid {
    ...
    /// v2.0.6 S3: plant seeded grass clumps from `world_seed`.
    ///
    /// Picks `clump_count` clump centres deterministically from `world_seed`
    /// (via the position-addressable hash RNG — independent of the sim RNG so
    /// creature draws are unaffected) and fills every cell within `clump_size`
    /// cells (Euclidean disc) to its biome carrying capacity. The disc fill is
    ...
    pub fn seed_clumps(&mut self, world_seed: u32, clump_count: u32, clump_size: u32) {
        ...
        let dim = self.dims.grass_dim as i64;
        let wrap = self.dims.wrap_world;
        let r = clump_size as i64;
        let r_sq = r * r;

        for ci in 0..clump_count {
            // Derive clump centre from world_seed + clump index via the stateless
            // hash RNG (salt 0 = cx, salt 1 = cy).
            let cx_raw = grass_hash_u64(world_seed, ci as u64, 0, 0);
            let cy_raw = grass_hash_u64(world_seed, ci as u64, 0, 1);
            let cx = (cx_raw % (dim as u64)) as i64;
            let cy = (cy_raw % (dim as u64)) as i64;

            // Fill every cell within the Euclidean disc of radius `clump_size`.
            for dy in -r..=r {
                for dx in -r..=r {
                    if dx * dx + dy * dy > r_sq {
                        continue;
                    }
                    // Compute cell coordinates with optional wrap.
                    let (gx, gy) = if wrap {
                        (
                            (cx + dx).rem_euclid(dim) as usize,
                            (cy + dy).rem_euclid(dim) as usize,
                        )
                    } else {
                        let gx = cx + dx;
                        let gy = cy + dy;
                    ...
                    let idx = gy * self.dims.grass_dim + gx;
                    // Seed to the cell's own biome carrying capacity.
                    let cap = self.capacity[idx];
                    if cap > 0.0 {
                        *self.density[idx].get_mut() = encode_density(cap);
                    }
                }
            }
        }
        // Rebuild bookkeeping so the first tick is correct. Callers may invoke
        // this once at the end of construction rather than after each batch, but
