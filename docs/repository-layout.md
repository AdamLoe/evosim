...
| `app/crates/evosim/src/*_tests.rs`, `app/crates/evosim/src/world/*_tests.rs` | Unit-test modules: `brain_width_tests`, `grass_v201_tests` (active-tile equivalence + dirty-tile quantize for the v2.0.1 grass rewrite), `wasm_dims_tests`, `wasm_snapshot_tests`; `biome_tests`, `genome_tests`, `grass_biome_tests`, `mating_tests`, `mating_grid_regression_tests`, `seeding_tests`, `wrap_tests`. |
