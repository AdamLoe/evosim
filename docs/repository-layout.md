...
| `src/wasm_api.rs` | `WorldHandle` — the sole wasm-bindgen surface. Owns `step_n`, `write_snapshot_to`, `set_slider`, `SLIDER_NAMES`, the genome→color + render-packing helpers (`genome_color_u32`, `pack_render_u32`, `quantize_grass_into`), the `biome_buf_byte_offset/len` getters, the polled `species_table_json` report, inspector + profile getters. Plus free functions `max_pop_for_sim()` and `rayon_current_num_threads()`. |
...
| `src/grass.rs` | `GrassGrid` — runtime-sized density Vec (`grass_dim²`, 5u cells), biome-capacity-scaled logistic in-cell growth + cross-kernel propagation, per-row "any non-empty" bitset, atomic per-tick timers. |
...
| `src/*_tests.rs`, `src/world/*_tests.rs` | Unit-test modules: `brain_width_tests`, `wasm_dims_tests`, `wasm_snapshot_tests`; `biome_tests`, `genome_tests`, `grass_biome_tests`, `mating_tests`, `mating_grid_regression_tests`, `seeding_tests`, `wrap_tests`. |
