# Prompts index

Reusable prompt templates and thin pointers.

## How to use this folder

Prompts own no project facts. If a prompt starts to carry substantive
system knowledge, move that content into `architecture/` or
`agent-context/` and link to the owner.

## Prompt map

| Need | Read |
|---|---|
| Fresh-chat entry point | [`fresh-chat.md`](fresh-chat.md) |
| Dev-server/startup prompt | [`dev-server.md`](dev-server.md) |
| Periodic docs-vs-code drift audit | [`check-docs.md`](check-docs.md) |
| End-of-chat wrap-up (plans, docs, commit, worktrees, agents) | [`wrap-up.md`](wrap-up.md) |

## See also

- [`../index.md`](../index.md)
- [`../ownership.md`](../ownership.md)
