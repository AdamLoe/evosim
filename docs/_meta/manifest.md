...
and rules in `~/.claude/agent-docs/<agent_docs_version>/` read the slots
...
| Plan lifecycle or status-metadata shape | `~/.claude/agent-docs/v1/plan-lifecycle.md` + `plan-template.md` (generic, in the kit); `plans/index.md` only if the app's landing/routing changes |
...
- The generic agent-docs kit (authoring rules, coding-style, repo-rules,
  orchestrating rules) lives at `~/.claude/agent-docs/v1/rules/`. The
