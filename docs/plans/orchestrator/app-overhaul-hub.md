...
- `24539da` reviewed wave 1 and fixed stale overview/manifest docs.
  Verification reported by reviewer: `cd app/web && pnpm docs:lint`,
...
- `05-world-config-schema-presets.md` shipped Rust-owned `WorldConfig`,
  generated TS defaults/types/presets, config-object worker boot, settings
  migration, and master-seed derivation. Verification reported by implementer:
...
- `04-world-persistence-artifacts.md` shipped world persistence/autosave/
  forking/import/export artifacts. Verification reported by implementer:
  Rust artifact round-trip tests, full Rust lib tests with and without
  `threads`, both clippy feature sets, wasm rebuild, web typecheck, docs lint,
  focused and full Playwright e2e, web build, `cargo bench --no-run`, and
  `git diff --check`.
